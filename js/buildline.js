...
var svgLine = d3.select('#buildline').append("svg")
...
d3.csv("/csv/buildpermits.csv", function(error, data) {
  ...
  var sectors = color.domain().map(function(name) {
    return {
      ...
      values: data.map(function(d) {
        return {date: d.date, Permits: +d[name]};
      })
    };
  });
  ...
  y.domain([
    d3.min(sectors, function(c) { return d3.min(c.values, function(v) { return v.Permits; }); }),
    d3.max(sectors, function(c) { return d3.max(c.values, function(v) { return v.Permits; }); })
  ]);
  ...
  city.append("text")
      ...
      .attr("x", 3)
      .attr("dy", ".35em")
      .text(function(d) { return '$ ' + d3.round(d.value.Permits*0.01, 2) + ' billion' });
